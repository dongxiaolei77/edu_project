...
use edu_ods;
-- 建表  ods层
drop table if exists edu_ods.n_web_chat_ems_2019_07;
create table if not exists edu_ods.n_web_chat_ems_2019_07(    		-- 访问咨询主表
        id   	                        int						comment '主键',
        create_date_time                timestamp   			comment '数据创建时间',
        session_id                      string  			comment '会话系统sessionId',
        sid                             string       	    comment '访客id',
        create_time                     date    			comment '会话创建时间',
        ...
        last_customer_msg_time_stamp    date	            comment '客户最后一条消息的时间',
        last_agent_msg_time_stamp       date	            comment '坐席最后一下回复的时间',
...
drop table if exists edu_ods.n_web_chat_text_ems_2019_07;
create table if not exists edu_ods.n_web_chat_text_ems_2019_07(		-- 访问咨询记录附属表（一对一关系）
        id                   	int		    comment '主键',
...
-- 来源：scrm
drop table if exists edu_ods.s_customer ;
create table if not exists edu_ods.s_customer(						-- 客户静态信息表（关联获取客户的静态信息，比如地区信息）
        id      int,
...
drop table if exists edu_ods.s_customer_appeal;
create table if not exists edu_ods.s_customer_appeal(				-- 线索申诉表（主要来判断客户线索被投诉无效）
        id                                     int,
...
drop table if exists edu_ods.s_customer_clue;
create table if not exists edu_ods.s_customer_clue(					-- 客户线索表  -- 拉链表
        id  int,
        create_date_time		    date		        comment '创建时间',
        update_date_time	        timestamp			comment '最后更新时间',
        deleted				        int			        comment '是否被删除（禁用）',
...
drop table if exists edu_ods.s_customer_relationship;
create table if not exists edu_ods.s_customer_relationship(			-- 客户意向表  -- 拉链表
        id                          int,
...
drop table if exists edu_ods.s_employee;
create table if not exists edu_ods.s_employee(						-- 员工信息表
        id                      int,
...
drop table if exists edu_ods.s_itcast_clazz;
create table if not exists edu_ods.s_itcast_clazz(					-- 报名课程表（获取客户报名的校区和学科）
...
drop table if exists edu_ods.s_itcast_school;
create table if not exists edu_ods.s_itcast_school(				-- 学校表
        id                      int,
        create_date_time 		date		    comment '创建时间',
        update_date_time 		timestamp		comment '最后更新时间',
        deleted          		int		        comment '是否被删除（禁用）',
...
drop table if exists edu_ods.s_itcast_subject;
create table if not exists edu_ods.s_itcast_subject(				-- 学科表
        id                  int,
        create_date_time	date			    comment '创建时间',
        update_date_time	timestamp			comment '最后更新时间',
        deleted     		int			        comment '是否被删除（禁用）',
...
drop table if exists edu_ods.s_scrm_department;
create table if not exists edu_ods.s_scrm_department(				-- 部门表
        id                      int,
...
-- 来源：teach
drop table if exists edu_ods.t_calendar;
create table if not exists edu_ods.t_calendar(
        id		    int				comment '主键',
        datelist    date
)
partitioned by (dt string)
...
drop table if exists edu_ods.t_class_studying_student_count;
create table if not exists edu_ods.t_class_studying_student_count(	-- 班级在读学生人数
        id                          int,
...
drop table if exists edu_ods.t_course_table_upload_detail;
create table if not exists edu_ods.t_course_table_upload_detail(	-- 班级课表
        id						int		    comment 'id',
...
drop table if exists edu_ods.t_student_leave_apply;
create table if not exists edu_ods.t_student_leave_apply(			-- 学生请假申请表
        id                  int,
        class_id        	int			comment '班级id',
...
drop table if exists edu_ods.t_tbh_class_time_table;
create table if not exists edu_ods.t_tbh_class_time_table(			-- 班级作息时间表
        id							int	            comment '主键id',
        class_id              		int	            comment '班级id',
...
drop table if exists edu_ods.t_tbh_student_signin_record;
create table if not exists edu_ods.t_tbh_student_signin_record(		-- 学生打卡记录表
        id            			int		comment '主键id',
